...
public class QuickServerApplication extends MultiThreadedApplicationAdapter {
	private static Logger log = LoggerFactory
			.getLogger(QuickServerApplication.class);
	...
	private String parseGroupId(String scopeContext) {
		String groupId = "";
		if (scopeContext != null && scopeContext.length() > 0) {
			if (scopeContext.startsWith("/")) {
				groupId = scopeContext.substring(1);
				return groupId;
			}
		}
		return groupId;
	...
	public boolean appConnect(IConnection conn, Object[] args) {
		...
		if (args.length != 1) {
			System.err.println("Only one argument is expected, but it's "
					+ args.length);
			return false;
		}
	...
	/**
	 * Called when an application is destroyed, this occurs when an application
	 * is stopped.
	 */
	@Override
	...
	public void streamBroadcastStart(IBroadcastStream stream) {
		...
		broadcast(streamStartMessage, stream.getPublishedName(),
				stream.getScope());
		
		String groupId = parseGroupId(scopeContext);
		String userName = stream.getPublishedName();
		AppMessageSender sender = QSBeanLoader.getAppMessageSender();
		sender.notifyUserVideoOn(groupId, userName);
	...
	public void streamBroadcastClose(IBroadcastStream stream) {
		...
		broadcast(streamCloseMessage, stream.getPublishedName(),
				stream.getScope());
		
		String groupId = parseGroupId(scopeContext);
		String userName = stream.getPublishedName();
		AppMessageSender sender = QSBeanLoader.getAppMessageSender();
		sender.notifyUserVideoOff(groupId, userName);
	}
	...
	private void callClientFunc(String func, Object message, IClient client) {
		...
		if (connSet == null) {
			log.debug("client " + client.getAttribute(UserID) + " scope "
					+ scope.getContextPath() + " has NULL connections.");
			return;
		}
